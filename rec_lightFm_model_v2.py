...
class LightFmModel:
    def __init__(self, user_features_file, item_features_file, item_user_iter_file, item_id, user_id):
        self._item_features_file = item_features_file
        self._item_user_iter_file = item_user_iter_file
        self._user_features_file = user_features_file
        self.n_users = 0
        self.n_items = 0
        self.tag=None
        self.tar_cols =None
    ...
    def load_data(self):
        # load datasets
        if self._item_features_file:
            try:
                self._item_features = pd.read_excel(os.path.join('data', self._item_features_file))
            except:
                self._item_features = pd.read_csv(os.path.join('data', self._item_features_file))
        if self._item_user_iter_file:
            try:
                self._item_user_interactions = pd.read_excel(os.path.join('data', self._item_user_iter_file))
            except:
                self._item_user_interactions = pd.read_csv(os.path.join('data', self._item_user_iter_file))
        if self._user_features_file:
            try:
                self._item_user_features = pd.read_excel(os.path.join('data', self._user_features_file))
            except:
                self._item_user_features = pd.read_csv(os.path.join('data', self._user_features_file))


    def clean_data(self, item_cat_cols, remove_cols_list, tag):
        ...
        self.tar_cols = numeric_item_features_clean.columns

        item_features_clean.rename(columns = {self.item_id:'business_name'}, inplace=True)
        ...
        item_user_visits.set_index(self.user_id, inplace=True)
        ...
        add_cols_list = ['business_name', self.user_id]
        ...
        item_users_interactions.rename(columns={self.user_id: 'user_name'}, inplace=True)
        ...
        item_users_interactions.to_csv('data/restaurant/interactions.csv')
    ...
    def dataset_model(self, data_user, item_users_interactions, item_features_clean):
        ...
        item_features = dataset.build_item_features(((x['business_name'],
                                                       ...
                                                       **self.build_dict(x, self.tar_cols, [1 * x['visits_count'] / max_b_rc])})
        ...
        #     print('Test interactions matrix and train interactions matrix share 1 interactions')
    ...
    @staticmethod
    def lightfm_model_hybrid(train, test, user_features, item_features, loss_mod='warp', num_threads=4, num_components=21,
    ...
    def similarity_user_items_ranks(model, train, test, data_meta, user_ids,
        ...
        for user_name, user_id in user_ids.items():
            ...
            for c in top_items:
                # print(f'(user_id: {user_id}, c: {c} ) = {user_id_score.loc[c, "scores"]}')
                # print(f'user : {user_name}')
                user_items_rank.loc[user_id, c] = user_id_score.loc[c, 'scores'].max()
    ...
    def test_rec(self, model, dataset, train, test, user_features, item_features, data_business, target_ls):
        rf.sample_train_recommendation(model, train, data_business, [1], 5, 'business_name',
                                       mapping=dataset.mapping()[2], tag=self.tag,
        ...
        rf.sample_test_recommendation(model, train, test, data_business, [5], 5, 'business_name',
                                      ...
                                      train_interactions=train, tag=self.tag, user_features=user_features,
                                      item_features=item_features)
        ...
        user_items_rank = self.similarity_user_items_ranks(model, train, test, data_business,
                                                      ...
                                                      train_interactions=train, tag=self.tag,
                                                      user_features=user_features,
                                                      item_features=item_features)
    ...
    def plot_2d_umap(self, df, features):
        ...
        p = umap.plot.interactive(mapper, labels=df[self.tag].fillna('None'), hover_data=hover_data, point_size=5)
        umap.plot.show(p)

    def plot_3d_umap(self, df, features):
        ...
        fig = px.scatter_3d(df_3d, x='x', y='y', z='z',
                            ...
                            color=df[self.tag].fillna('None'))
        pio.renderers.default = "browser"
        fig.show()
    def plot_rec(self, df_similarities_places, data_business, dataset, model, item_features_clean):
        ...
        item_embeddings_df = item_embeddings_df.merge(item_features_clean[['business_name', self.tag]],
                                                      on='business_name')
...
def dummy_dataset():
    ...
    item_id ='Name '
    user_id = 'Name '
    prep = LightFmModel(user_features_file,item_features_file, item_user_iter_file, item_id, user_id)
    # Load data
    prep.load_data()

    # Clean Data
    remove_cols_list = ['Name ', 'Category', 'SubCategory', 'Cuisine']
    item_cat_cols = ['Category', 'SubCategory']

    item_users_interactions, item_features_clean, data_user = prep.clean_data(item_cat_cols,remove_cols_list, tag= 'Category')
    ...
    dataset, data_business, train, test, user_features, item_features = prep.dataset_model(data_user,
    ...
    # warp
    train_auc, test_auc, model_warp = prep.lightfm_model(train, test, loss_mod='warp', num_threads=25,
                                                         num_components=21,
                                                         num_epochs=16, item_alpha=5.97967e-6, lr=0.033)

    train_auc, test_auc, model_hybrid_warp = prep.lightfm_model_hybrid(train, test, user_features, item_features, loss_mod='warp')

    logger.info('# -------------------------------------------------------------------------------------- #')
    logger.info('# --------------------------- Test Recommendation -------------------------------------- #')
    logger.info('# -------------------------------------------------------------------------------------- #')
    target_ls = ['WiFi', 'DJs', 'Crafty Egg']
    user_items_rank, df_similarities, df_similarities_places = prep.test_rec(model_hybrid_warp, dataset, train, test,
                                                                             user_features, item_features,
                                                                             data_business, target_ls)
    print(user_items_rank)

    logger.info('# -------------------------------------------------------------------------------------- #')
    logger.info('# ------------------------ Plot uma Representations ------------------------------------ #')
    logger.info('# -------------------------------------------------------------------------------------- #')
    prep.plot_rec(df_similarities_places, data_business, dataset, model_hybrid_warp, item_features_clean)

def restaurant_recommendation():
    logger.info('# -------------------------------------------------------------------------------------- #')
    logger.info('# --------------------------- Preprocessing Data --------------------------------------- #')
    logger.info('# -------------------------------------------------------------------------------------- #')
    item_features_file = 'bristol_poi_elite_datasheet_v1.0.csv'
    item_user_iter_file = 'user_poi_interactions.xlsx'
    user_features_file =  'user_attributes.xlsx'

    item_id= 'gandalfDestination.externalId'
    user_id= 'user'
    recom = LightFmModel(user_features_file,item_features_file, item_user_iter_file, item_id, user_id)
    # Load data

    recom.load_data()

    # Clean Data
    remove_item_cols_list = ['Unnamed: 0',
                             'gandalfDestination.businessStatus',
                             'gandalfDestination.city',
                             'gandalfDestination.displayName',
                             'gandalfDestination.formattedAddress',
                             'gandalfDestination.externalId',
                             'gandalfDestination.lat',
                             'gandalfDestination.lng',
                             'gandalfDestination.number',
                             'gandalfDestination.openingHours',
                             'gandalfDestination.openingHoursFormatted',
                             'gandalfDestination.photos',
                             'gandalfDestination.postcode',
                             'gandalfDestination.priceLevel',
                             'gandalfDestination.rating',
                             'gandalfDestination.types',
                             'gandalfDestination.userRatingsTotal',
                             'gandalfDestination.website',
                             'destination.name',
                             'category.name',
                             'subcategory.name',
                             'destination.postcode',
                             'destination.instagram',
                             'destination.bookATableUrl',
                             'cuisine.name',
                             'destination.other']

    item_cat_cols = ['category.name', 'subcategory.name']

    item_users_interactions, item_features_clean, data_user = recom.clean_data(item_cat_cols, remove_item_cols_list, tag= 'category.name')
    ...
    # Build Lightfm Dataset
    dataset, data_business, train, test, user_features, item_features = recom.dataset_model(data_user,
                                                                                           item_users_interactions,
                                                                                           item_features_clean)
    ...
    # Logistic
    train_auc, test_auc, model_log = recom.lightfm_model(train, test, loss_mod='logistic')

    # brp
    train_auc, test_auc, model_brp = recom.lightfm_model(train, test, loss_mod='bpr', num_threads=25, num_components=45,
                                                        num_epochs=43, item_alpha=1.3846e-6, lr=0.016144)
    # warp
    train_auc, test_auc, model_warp = recom.lightfm_model(train, test, loss_mod='warp', num_threads=25,
                                                         num_components=21,
                                                         num_epochs=16, item_alpha=5.97967e-6, lr=0.033)

    train_auc, test_auc, model_hybrid_warp = recom.lightfm_model_hybrid(train, test, user_features, item_features, loss_mod='warp')

    logger.info('# -------------------------------------------------------------------------------------- #')
    logger.info('# --------------------------- Test Recommendation -------------------------------------- #')
    logger.info('# -------------------------------------------------------------------------------------- #')

    target_ls = ['ChIJ0eTcMgWPcUgRQ39rYzEGkj8','ChIJn-ViznePcUgRnEkU_ERw1Nw','ChIJMWnHX5WNcUgRw4RsYle-Jbo']
    user_items_rank, df_similarities, df_similarities_places = recom.test_rec(model_hybrid_warp, dataset, train, test,
                                                                             user_features, item_features,
                                                                             data_business, target_ls)
    print(user_items_rank)
    ...
    logger.info('# ------------------------ Plot uma Representations ------------------------------------ #')
    logger.info('# -------------------------------------------------------------------------------------- #')
    recom.plot_rec(df_similarities_places, data_business, dataset, model_hybrid_warp, item_features_clean)

if __name__ == "__main__":
    #%% Run dummy dataset (demo)
    # dummy_dataset()


    #%% Run new dataset (Bristol)
